...
func TestAdmissionPolicyUpdateRequiresConfirmation(t *testing.T) {
	...
	updateReq := httptest.NewRequest(http.MethodPut, "/api/v1/admin/admission/policy", strings.NewReader(`{"policy_type":"approval","confirmation_token":"`+confirmation.ConfirmationToken+`"}`))
