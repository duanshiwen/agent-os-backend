...
func Setup(
	...
	skillHubRepo := repository.NewSkillHubRepo(db)
	skillHubSvc := service.NewSkillHubService(skillHubRepo)
	...
	skillHubSvc.SetSyncService(syncSvc)
	skillHubSvc.SetAuditService(auditSvc)
	skillHubSvc.SetObjectService(objectSvc)
	...
	skillHubH := handler.NewSkillHubHandler(skillHubSvc)
	...
	v1 := r.Group("/api/v1")
	{
		...
		v1.GET("/skills/catalog", skillHubH.SearchCatalog)
		v1.GET("/skills/catalog/:skill_key", skillHubH.GetCatalogSkill)
		...
		protected.Use(middleware.JWTAuth(cfg.JWT.Secret))
		{
			...
			protected.PATCH("/conversations/:id", convH.Update)
			protected.POST("/conversations/:id/read-state", convH.MarkRead)
			...
			protected.POST("/conversations/:id/messages/:message_id/reactions", convH.AddReaction)
			protected.DELETE("/conversations/:id/messages/:message_id/reactions/:emoji", convH.RemoveReaction)
			protected.POST("/conversations/:id/participants", convH.AddParticipant)
			protected.PATCH("/conversations/:id/participants/:user_id", convH.UpdateParticipant)
			...
			protected.POST("/skills", skillHubH.CreateSkill)
			...
			protected.POST("/skills/installations/:installation_id/disable", skillHubH.DisableInstallation)
			protected.DELETE("/skills/installations/:installation_id", skillHubH.UninstallSkill)
		...
		admin.Use(middleware.AdminMiddleware(userRepo))
		{
			...
			admin.POST("/skills/:skill_id/takedown", skillHubH.TakedownSkill)
			admin.POST("/skills/publishers/:publisher_id/restrict", skillHubH.RestrictPublisher)
			admin.POST("/skills/publishers/:publisher_id/lift-restriction", skillHubH.LiftPublisherRestriction)
